...
#if 0
        if(mr & MR_FS)
            pack_len = (((uint16_t)head[1]) << 8) | ((uint16_t)head[0]);
         else
            pack_len = (((uint16_t)head[0]) << 8) | ((uint16_t)head[1]);
#endif
        pack_len -= 2;
